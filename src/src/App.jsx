...
import { Dumbbell, TrendingUp, ShoppingBag, Camera, Plus, Trash2, ChevronRight, Search, Play, Square, LogOut, Crown, FolderPlus } from "lucide-react";
...
const STRIPE_LINK = "https://buy.stripe.com/test_3cI00k1HV0x65Mbc9B93y00";
...
function PremiumBanner({ userId }) {
  const link = `${STRIPE_LINK}?client_reference_id=${userId}`;
  return (
    <div style={{ background: `linear-gradient(135deg, ${C.yellow}22, ${C.amber}11)`, border: `1px solid ${C.yellow}55`, borderRadius: 14, padding: 16, marginBottom: 16, display: "flex", alignItems: "center", gap: 12 }}>
      <Crown size={24} color={C.yellow} />
      <div style={{ flex: 1 }}>
        <div style={{ color: C.cream, fontSize: 13, fontWeight: 700 }}>Vire Premium</div>
        <div style={{ color: C.gray, fontSize: 11 }}>Salve múltiplos planos de treino por R$9,50/mês</div>
      </div>
      <a href={link} target="_blank" rel="noopener noreferrer" style={{ ...primaryBtnStyle, textDecoration: "none", padding: "8px 14px", whiteSpace: "nowrap" }}>
        Virar Premium
      </a>
    </div>
  );
...
function PlanosBar({ planos, planoAtivo, setPlanoAtivo, isPremium, onCriarPlano }) {
  const [criando, setCriando] = useState(false);
  const [nomeNovo, setNomeNovo] = useState("");

  function confirmarCriar() {
    if (!nomeNovo.trim()) return;
    onCriarPlano(nomeNovo.trim());
    setNomeNovo("");
    setCriando(false);
  }
  ...
  return (
    <div style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap", alignItems: "center" }}>
      {planos.map((p) => (
        <button key={p.id} onClick={() => setPlanoAtivo(p.id)} style={{
          padding: "6px 14px", borderRadius: 20, border: `1px solid ${C.yellow}44`, cursor: "pointer",
          background: planoAtivo === p.id ? C.yellow : "transparent", color: planoAtivo === p.id ? C.black : C.cream,
      ...
      {isPremium && !criando && (
        <button onClick={() => setCriando(true)} style={{ ...ghostBtnStyle, padding: "6px 12px" }}>
          <FolderPlus size={14} /> Novo plano
        </button>
      )}
      {isPremium && criando && (
        <div style={{ display: "flex", gap: 6 }}>
          <input value={nomeNovo} onChange={(e) => setNomeNovo(e.target.value)} placeholder="Nome do plano" style={{ ...inputStyle, width: 140 }} autoFocus />
          <button onClick={confirmarCriar} style={{ ...primaryBtnStyle, padding: "8px 12px" }}>OK</button>
        </div>
      )}
    </div> function TreinoTab({ userId, isPremium, planos, planoAtivo, setPlanoAtivo, onCriarPlano, workouts, refresh }) {
  ...
  const workoutsDoPlano = workouts.filter((w) => w.plan_id === planoAtivo);

  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: 16 }}>
      {!isPremium && <PremiumBanner userId={userId} />}

      <PlanosBar planos={planos} planoAtivo={planoAtivo} setPlanoAtivo={setPlanoAtivo} isPremium={isPremium} onCriarPlano={onCriarPlano} />
      ...
      {workoutsDoPlano.length === 0 && <p style={{ color: C.gray, fontSize: 13 }}>Nenhum treino registrado ainda.</p>}
      {workoutsDoPlano.slice(0, 8).map((w) => (
...
const DICAS = {
  ...
  prancha: ["Não deixe o quadril subir ou cair.", "Contraia o abdômen e glúteo.", "Respire de forma constante, sem prender o ar."],
};
...
export default function App() {
  ...
  async function refresh() {
    if (!session) return;
    const { data: perfilData } = await supabase.from("profiles").select("*").eq("id", session.user.id).maybeSingle();
    setProfile(perfilData);

    let { data: planosData } = await supabase.from("user_plans").select("*").eq("user_id", session.user.id).order("created_at");
    if (!planosData || planosData.length === 0) {
      const { data: novoPlano } = await supabase.from("user_plans").insert({ user_id: session.user.id, nome: "Meu treino" }).select().single();
      planosData = [novoPlano];
    }
    setPlanos(planosData);
    setPlanoAtivo((atual) => atual || planosData[0]?.id);

    const { data: workoutsData } = await supabase.from("workouts").select("*").eq("user_id", session.user.id).order("data", { ascending: false });
  ...
  async function criarPlano(nome) {
    const { data } = await supabase.from("user_plans").insert({ user_id: session.user.id, nome }).select().single();
    if (data) {
      setPlanos((p) => [...p, data]);
      setPlanoAtivo(data.id);
    }
  }
